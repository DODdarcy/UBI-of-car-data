...
dbscan = DBSCAN(eps = 0.4,min_samples=3, metric='euclidean').fit(dataxl_true)


res = dbscan.labels_
pd.concat([pd.DataFrame(dataxl_true),pd.DataFrame(res)],axis=1).to_excel("DBSCAN聚类结果.xlsx")
print(res)
